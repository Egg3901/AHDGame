...
Status: proposed · 2026-09-25. This file supersedes and expands an earlier
...
execution plan; each work package lists its own gates.
...
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Turn 960 failed at 204.9 s against a "hard 240 s ceiling"; p95 191.5 s leaves 20% headroom | `PHASE_TIMEOUT_MS = 4 * 60 * 1000` is a **per-phase** timeout (`src/lib/turn/processingLock.ts:23`), applied per phase in `turnPhaseRuntime.ts` (~line 233, `Promise.race` against the phase function). There is no whole-turn wall clock; `TURN_LOCK_STALE_MS` (20 min) governs lock staleness. Whole-turn 204.9 s cannot itself hit a 240 s per-phase limit. [code]                                                                                                                                                                                                                                                                                               | Attribute turn 960 from its per-phase status and error in `turnLogs`; separately define a whole-turn SLO, the per-phase timeout, and lock-staleness semantics. |
...
- **`shareOrders` lacks a fund-bid index.** The index-fund bid sweep filters
  `{ placerFundId, type: "buy", status: "open" }` twice per fund
  (`fundCron.ts` ~lines 783, 822) against a ~294 k-row collection whose seeded
  indexes lead with `corporationId` or `characterId` (`performance.ts` ~460-483).
  No index leads with `placerFundId`. [code]
...
8. **Index design and data lifecycle.** Collection counts say nothing about
   ...
   `shareOrders` lacks a `placerFundId` index; `turnLogs` has query indexes but
   no TTL in this checkout.
...
- **Surface:** `src/lib/indexFunds/fundCron.ts` (passes 1, 3, 3c, step 8),
  ...
  `fundCron.ts` (~253, ~942).
- **Smallest useful steps:**
  1. Add a `{placerFundId: 1, type: 1, status: 1}` index on `shareOrders` -
     the current scans have no leading index on a ~294 k-row collection.
     Cheap, independent, shippable alone.
  2. One `find({_id: {$in: fundIds}})` per pass; carry the fund document
     through the sell loop instead of re-reading.
  3. Hoist the duplicate open-bid scan: read once per fund, diff in memory
     after cancellations.
...
NPP command-core locations and guarded writes, `shareOrders` index gap,
budget values, phase-name counts, and the typecheck heap ceiling.

Still requires production telemetry or a live database: turn 960's actual
failure phase and error; current whole-turn and per-phase distributions;
current command counts; `electionCandidates` count and status breakdown;
document counts per collection; whether production runs the turn via
`CRON_OWNER=worker`; real tsc peak RSS; the true per-turn executed-phase
count per world.

Not done here: the operations page is unchanged (no authorized edit path was
used); the 11 streams are planned, not implemented; no measurements were
re-run (WP0 is the first work package precisely because the baseline needs
fresh provenance).
