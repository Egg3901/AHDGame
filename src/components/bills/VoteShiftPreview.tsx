...
function describeVote(delta: AxisDelta): string {
  if (delta.economic === 0 && delta.social === 0) {
    return "no change, you already hold this position";
  }
...
export function VoteShiftPreview({
  ...
  return (
    <div className={`space-y-0.5 text-xs text-muted ${className}`.trim()}>
      <p>
        <span className="font-semibold text-success">Aye:</span> {describeVote(preview.aye)}
      </p>
      <p>
        <span className="font-semibold text-error">Nay:</span> {describeVote(preview.nay)}
      </p>
    </div>
  );
}
