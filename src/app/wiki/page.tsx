...
export default async function WikiOverviewPage({
  ...
  const [allPages, autoGeneratedPages, recentPages, mostViewedPages, user, learningPaths] =
    await Promise.all([
      getAllWikiPagesForDisplay(),
      getAutoGeneratedWikiPagesForDisplay(),
      getRecentWikiPages(),
  ...
  const guidePages = allPages
    .filter(
      (p) =>
        !p.isAutoGenerated &&
        !p.isCustomPage &&
        (p.contentType === "guide" || p.contentType === "strategy")
    )
    .sort((a, b) => a.title.localeCompare(b.title));
              ...
              {guidePages.length > 0 ? (
                <div className="grid gap-2 sm:grid-cols-2">
                  {guidePages.map((page) => (
                    <Link
                      key={page.slug}
                      href={page.href ?? `/wiki/${page.slug}`}
                      className="group rounded-lg border border-card-border bg-card/40 px-4 py-3 transition-colors hover:border-primary/50 hover:bg-card-elevated"
                    >
                ...
                <p className="text-sm text-muted/60">No guides yet.</p>
              )}
            </section>
