...
function ScotusNominationDetailContent() {
  ...
  const fetchNom = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/congress/scotus-nominations/${id}`);
      if (!res.ok) {
  ...
  async function handleVote(vote: "for" | "against" | "abstain") {
    ...
    setVoting(true);
    try {
      const res = await fetch(`/api/whitehouse/scotus/nominations/${id}/vote`, {
        method: "POST",
  ...
  const scotusHref = `/country/${nom.countryId.toLowerCase()}/executive/supreme-court`;
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="rounded-xl border border-card-border bg-card shadow-panel p-6 space-y-4">
          ...
          <p className="text-sm text-muted leading-relaxed">
            Supreme Court nomination · Senate confirmation required. Simple majority within 24
            hours.
          </p>
        ...
        <div className="rounded-xl border border-card-border bg-card shadow-panel p-5 space-y-4">
          ...
          {!nom.isSenator && (
            <p className="text-xs text-muted pt-1">
              Only Senators can vote on Supreme Court nominations.
            </p>
          )}
        </div>

        <Link href={scotusHref} className="inline-block text-sm text-primary hover:underline">
          View Supreme Court →
        </Link>
      </main>
    </div>
...
export default function ScotusNominationDetailPage() {
  return (
    <NominationPageErrorBoundary>
      <ScotusNominationDetailContent />
    </NominationPageErrorBoundary>
  );
}
