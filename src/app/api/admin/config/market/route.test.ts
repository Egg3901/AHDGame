 ...
 */
describe("GET/PATCH /api/admin/config/market — corporationProductsEnabled", () => {
  let db: MockDb;
  ...
  it("GET reflects the flag and defaults it to false when absent", async () => {
    db.collectionMocks.gameConfig!.findOne.mockResolvedValue({
      _id: "default",
      marketSystemMode: "capital",
      corporationProductsEnabled: true,
    });
    const { GET } = await import("./route");
    const enabled = (await (await GET()).json()) as {
      corporationProductsEnabled: boolean;
    };
    expect(enabled.corporationProductsEnabled).toBe(true);
    ...
    expect(absent.corporationProductsEnabled).toBe(false);
  });

  it("PATCH persists explicit true/false via $set", async () => {
    const { PATCH } = await import("./route");

    const enabled = await PATCH(
      makePatchRequest({ mode: "capital", corporationProductsEnabled: true })
    );
    ...
    const disabled = await PATCH(
      makePatchRequest({ mode: "capital", corporationProductsEnabled: false })
    );
    ...
    const setArg = db.collectionMocks.gameConfig!.updateOne.mock.calls.at(-1)?.[1]?.$set as Record<
      ...
      unknown
    >;
    expect(setArg.corporationProductsEnabled).toBe(false);
  });

  it("PATCH omits the flag from $set when not provided", async () => {
    const { PATCH } = await import("./route");
    await PATCH(makePatchRequest({ mode: "capital" }));
    ...
    const setArg = db.collectionMocks.gameConfig!.updateOne.mock.calls[0]?.[1]?.$set as Record<
      ...
      unknown
    >;
    expect(setArg).not.toHaveProperty("corporationProductsEnabled");
  });

  it("PATCH rejects a non-boolean flag with 400 and writes nothing", async () => {
    const { PATCH } = await import("./route");
    const res = await PATCH(
      makePatchRequest({ mode: "capital", corporationProductsEnabled: "yes" })
    );
