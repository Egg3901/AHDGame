...
export async function POST(request: Request) {
  try {
    ...
    const crisisDefinition = warEmergencyTemplateForEventKind(kind);
    if (crisisDefinition) {
      ...
      if (alreadyActive) {
        throw conflict(`${crisisDefinition.template.name} is already active in ${countryId}`);
      }
      ...
      return NextResponse.json({
        success: true,
        crisis: {
          crisisId: crisisId.toHexString(),
          templateKey: crisisDefinition.templateKey,
          title: crisisDefinition.template.name,
