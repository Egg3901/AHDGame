...
const { cookieState, exchangeCodeForToken, fetchDiscordUser, getDb } = vi.hoisted(() => {
...
vi.mock("@/lib/discord", () => ({ exchangeCodeForToken, fetchDiscordUser }));
...
const userId = new ObjectId("0000000000000000000000aa");
const discordUser = { id: "discord-user-1", username: "alpha", avatar: null };
...
describe("GET /api/auth/discord/callback — existing-user reauth", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    cookieState.reset({
      discord_oauth_mode: "login",
    ...
    fetchDiscordUser.mockResolvedValue(discordUser);
  });
  ...
  it("keeps authRevokedAt, issues a strictly newer JWT, and sets the cookie only after a matching write", async () => {
    ...
    const res = await GET(
      new Request("https://ahousedividedgame.com/api/auth/discord/callback?code=abc&state=state-1")
    );
    ...
    expect(filter.discordId).toBe(discordUser.id);
