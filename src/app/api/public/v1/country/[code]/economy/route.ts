...
import { queryCountryEconomy } from "@/lib/publicApi/economy";

// GET /api/public/v1/country/[code]/economy
// Auth: PUBLIC_BOT_API_KEY
export async function GET(request: Request, { params }: { params: Promise<{ code: string }> }) {
  try {
    const guard = await publicApiGuard(request, "economy");
    if (!guard.ok) return guard.response;

    const { code } = await params;
    const db = await getDb();
    const result = await queryCountryEconomy(db, code.toUpperCase());
