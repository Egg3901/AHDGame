/**
 * POST /api/whitehouse/vp-nomination/[id]/withdraw — President withdraws VP nomination
 */
...
// POST /api/whitehouse/vp-nomination/[id]/withdraw — President withdraws a pending VP nomination.
  ...
  try {
    ...
    if (nomineeChar?.userId) {
      await createNotification({
        ...
        type: "system",
        title: "VP Nomination Withdrawn",
        message: `Your nomination for Vice President was withdrawn by the President.`,
        metadata: { nominationId: nominationOid.toString(), type: "vp_nomination_withdrawn" },
      });
    }

    return NextResponse.json({ success: true, message: "VP nomination withdrawn" });
  } catch (error) {
