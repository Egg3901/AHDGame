...
describe("standing targeted ad actions", () => {
  ...
  it("quotes shuffled demographic buckets in census dimension order", async () => {
    const cell = (
      id: string,
      ...
      identities: Object.fromEntries(
        Object.keys(buckets).map((dimension) => [dimension, { economicLean: 3, socialLean: 3 }])
      ),
    });
    vi.mocked(loadCampaignAudience).mockResolvedValue({
      cells: [
        cell("a", { race: "white", age: "mature", education: "no_college", wealth: "middle" }),
        cell("b", { race: "black", age: "mid", education: "college", wealth: "low" }),
        cell("c", { race: "white", age: "senior", education: "graduate", wealth: "high" }),
        cell("d", { race: "white", age: "young", education: "college", wealth: "middle" }),
      ],
      context: {
        countryId: "US",
        stateId: "CA",
    ...
    expect(
      body.targets.map(
        (target: { dimension: string; bucket: string }) => `${target.dimension}:${target.bucket}`
      )
    ).toEqual([
      "race:white",
      "race:black",
    ...
    expect(
      body.targets.find((target: { bucket: string }) => target.bucket === "young")
    ).toMatchObject({
      label: "Under 30s",
      dimLabel: "Age",
    });
