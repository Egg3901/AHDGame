...
function makeRequest(body: Record<string, unknown>) {
  return new Request("http://localhost/api/country/us/parties/1/registration-drive", {
    method: "POST",
...
describe("POST /api/country/[code]/parties/[id]/registration-drive (#81)", () => {
  let db: MockDb;
  ...
  it("does not reset budgets before rejecting an unauthorized caller", async () => {
    ...
    const response = await POST(makeRequest({ registrationBudgetPercent: 10 }), {
  ...
  it("allows an authorized chair to set the registration budget when treasury is positive", async () => {
    ...
    expect(body.registrationBudgetPercent).toBe(10);
    expect(db.collectionMocks["partyBudget"]!.updateOne).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        $set: expect.objectContaining({ registrationBudgetPercent: 10, countryId: "US" }),
  ...
  it("returns 200 when treasury is negative but percent is 0", async () => {
    ...
    const { POST } = await import("./route");
    const response = await POST(makeRequest({ registrationBudgetPercent: 0 }), {
      params: Promise.resolve({ code: "us", id: "1" }),
    });
