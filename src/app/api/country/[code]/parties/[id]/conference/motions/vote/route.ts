...
import { voteOnMotion } from "@/lib/uk/conference/conferenceCommands";

interface RouteParams {
  params: Promise<{ code: string; id: string }>;
}

const motionVoteSchema = z.object({
  motionId: z.string().min(1),
  vote: z.enum(["aye", "nay"]),
});

// POST /api/country/[code]/parties/[id]/conference/motions/vote — cast (or
// change) a committee vote on a rules motion. Committee only.
...
async function postHandler(request: Request, { params }: RouteParams) {
  try {
    ...
    const parsed = await parseJsonBody(request, motionVoteSchema);
    if (!parsed.success) {
    ...
    if (authUser.character.party !== partyId || !isSameCountry(authUser.character, party)) {
      return NextResponse.json(
        { error: "You must be a member of this party to vote on conference motions" },
        { status: 403 }
      );
    }

    const gameTime = await getGameTime();
    const result = await voteOnMotion(
      db,
      countryId,
      String(party.sequentialId),
      parsed.data.motionId,
      {
