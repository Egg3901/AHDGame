...
// POST /api/country/[code]/region/[id]/party/[partyId]/suppression — Set the state voter suppression budget percentage and targeting
  ...
  try {
    ...
    if (!isAdmin && !isNationalChair && !isStateChair && !isStateViceChair && !isStateTreasurer) {
      return NextResponse.json(
        {
          error:
            "Only the state chair, vice chair, treasurer, national chair, or an admin can set the suppression budget",
        },
        { status: 403 }
      );
    }
    ...
    await db.collection("adminLogs").insertOne({
      ...
      details: `State suppression budget for ${state.name} ${party.name} set to ${percent}%${suppressionTargetGroup ? ` targeting ${suppressionTargetGroup}` : ""}`,
      createdAt: now,
    });
