...
describe("FreightMarketExplainer", () => {
  it("explains cargo classes, shared capacity, and why sold percentages differ", () => {
    ...
    expect(screen.getByText(/not separate freight products or reserved capacity pools/i)).toBeTruthy();
    expect(screen.getByText(/three times as much TEU/i)).toBeTruthy();
    expect(screen.getByText(/sector's sold percentage shows only the share captured/i)).toBeTruthy();
  });
});
