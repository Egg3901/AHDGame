...
describe("linking back to the command's structure", () => {
  it("offers a link to where the command was built", () => {
    // A Command lives on two pages — built in the defence seat's office, employed
    // here. Without this the CG has no route to see their own command's makeup.
    render(
      <CommandingGeneralClient
        {...base}
        commandStructureHref="/country/us/executive/cabinet/secretary_of_defense/office#commands"
      />
    );
    const link = screen.getByRole("link", { name: /structure and units/i });
    expect(link.getAttribute("href")).toBe(
      "/country/us/executive/cabinet/secretary_of_defense/office#commands"
    );
  });
  ...
  it("deep-links to the Commands tab, not just the office", () => {
    ...
    expect(screen.getByRole("link", { name: /structure and units/i }).getAttribute("href")).toMatch(
      /#commands$/
    );
  });

  it("offers no link for a country with no defence seat", () => {
    // Better no link than one to an office that does not exist.
    render(<CommandingGeneralClient {...base} commandStructureHref={null} />);
    expect(screen.queryByRole("link", { name: /structure and units/i })).toBeNull();
  });
});
