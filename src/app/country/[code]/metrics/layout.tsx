...
export async function generateMetadata({
  ...
  const title = `${country.name} Metrics | A House Divided`;
  const description = `Economic, social, and demographic metrics for ${country.name} — national rollups and state rankings.`;
  const url = `${getSiteUrl()}/country/${code}/metrics`;
...
export default function CountryMetricsLayout({ children }: LayoutProps) {
  return <>{children}</>;
}
