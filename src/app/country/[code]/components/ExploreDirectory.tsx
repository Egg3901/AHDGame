 ...
 * nav-card grid with three group cards whose rows carry live figures.
          ...
          {group.rows.map((row) =>
            row.available ? (
              <Link
                key={row.label}
                href={row.href}
                className={`flex items-center justify-between gap-3 border-b border-card-border/50 px-4 py-2.5 transition-colors last:border-b-0 hover:bg-card-elevated/40 ${
              ...
              <div
                key={row.label}
                className="flex items-center justify-between gap-3 border-b border-card-border/50 px-4 py-2.5 opacity-60 last:border-b-0"
              >
