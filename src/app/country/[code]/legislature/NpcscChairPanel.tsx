...
interface NpcscChairResponse {
  ...
  /** True when the viewer's party is eligible under the chair_npcsc role policy. */
  canRun: boolean;
...
const ROLE_LABEL = "Chairman of the NPC Standing Committee";

/**
 * NPC Standing Committee Chairman panel — mirrors the US House Speaker / DE
 * Bundestagspräsident panel mechanic. Shows the current presiding officer, an
 * open election when active, and declare/vote/start buttons gated by NPC
 * membership + role eligibility. Viewer identity comes from the GET response
 * (server-side session lookup). Mounted on the CN NPC page.
 */
export function NpcscChairPanel() {
  const { showToast } = useToast();
  const clock = useGameClock();
  const [data, setData] = useState<NpcscChairResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);

  const fetchData = useCallback(async () => {
    try {
      const res = await fetch("/api/country/cn/legislature/npcsc-chair", {
        cache: "no-store",
      });
      if (res.ok) {
        setData((await res.json()) as NpcscChairResponse);
      }
    } catch {
    ...
    <div className="rounded-xl border border-card-border/40 bg-card-muted/30 p-4 space-y-3">
      ...
      {!isElectionActive && data.leader?.characterId == null && (
        <p className="text-xs text-muted/70">
          No Chairman of the NPC Standing Committee seated. An admin can open an election.
        </p>
      )}
    </div>
  );
