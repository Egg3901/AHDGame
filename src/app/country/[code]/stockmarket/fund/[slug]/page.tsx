...
import type { PriceChangeTimeframe } from "../../components/StockList";
...
function FundDetailPageInner({ params }: { params: Promise<{ code: string; slug: string }> }) {
  ...
  const stockMarketHref =
    code.toLowerCase() === "global"
      ? `/stockmarket/global?tab=funds`
      : `/country/${code.toLowerCase()}/stockmarket?tab=funds`;
...
function formatTurnFromDate(iso: string): string {
  ...
  // The detail payload doesn't include a turn number for the rebalance; the
  // surface just shows the timestamp rounded to date to keep the label short.
  const d = new Date(t);
