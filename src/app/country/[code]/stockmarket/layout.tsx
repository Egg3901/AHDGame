...
export async function generateMetadata({
  ...
  const title = `${country.name} Stock Market | A House Divided`;
  const description = `Listed corporations, market cap leaders, sector indices, and trading activity on the ${country.name} stock exchange.`;
  const url = `${getSiteUrl()}/country/${code}/stockmarket`;
...
export default function CountryStockmarketLayout({ children }: LayoutProps) {
  return <>{children}</>;
}
