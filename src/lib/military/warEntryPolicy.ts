...
export function classifyWarEntry(params: {
  ...
  const { conflict, countryId, side, organizationId, defendingCountryId } = params;
  if ((conflict.hostEntities ?? [conflict.hostCountry]).includes(countryId)) {
    return "principal_belligerent";
  }
  const designatedBloc = (BLOC_DESIGNATED_ORG_IDS as readonly string[]).includes(organizationId);
  if (!designatedBloc) {
