...
/**
 * The small facts about where RU is.
 *
 * ⚠ GENERATED FROM `__snapshots__/ru.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts RU --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps RU to the code and
...
export const RU_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const RU_ISO_NUMERIC = "643";

export const RU_UN_MEMBER_SINCE = 1945;

export const RU_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const RU_NPP_CAPITAL_STATE = "";

/* No RU_NON_PARTY_INDEPENDENT_BIAS: the registry has no RU entry, and
...
export const RU_MAP_ANCHOR: [number, number] = [37.6, 55.8];
 ...
 * another country's.
 */
/* No RU_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.RU`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No RU_CORE5_NORMALS: the metric-first registry carries no RU anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR RU THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const RU_ADJACENCY_MAP: AdjacencyMap = {
  CEN: ["NWR", "NOR", "VOL", "CBE"],
  NWR: ["CEN", "NOR"],
 ...
 */
export const RU_MAP_REGISTRY: CountryMapConfig = {
  countryId: "RU",
  name: "Russia",
 ...
 * string.
 */
export const RU_INCOME_ANCHORS: NormalAnchor[] = [
  {
    year: 1953,
    value: 10500,
  },
 ...
 */
/* No RU_POPULATION_MULTIPLIERS: the 1991 cohort table has no RU row, so that era passes through at 1.0. */
