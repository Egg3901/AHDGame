...
/**
 * The small facts about where SE is.
 *
 * ⚠ GENERATED FROM `__snapshots__/se.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts SE --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps SE to the code and
...
export const SE_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const SE_ISO_NUMERIC = "752";

export const SE_UN_MEMBER_SINCE = 1946;

export const SE_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const SE_NPP_CAPITAL_STATE = "SE_STH";

/* No SE_NON_PARTY_INDEPENDENT_BIAS: the registry has no SE entry, and
...
export const SE_MAP_ANCHOR: [number, number] = [18.6, 60.1];
 ...
 * another country's.
 */
/* No SE_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.SE`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No SE_CORE5_NORMALS: the metric-first registry carries no SE anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR SE THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const SE_ADJACENCY_MAP: AdjacencyMap = {
 ...
 */
export const SE_MAP_REGISTRY: CountryMapConfig = {
  countryId: "SE",
  name: "Sweden",
 ...
 * string.
 */
export const SE_INCOME_ANCHORS: NormalAnchor[] = [
  {
    year: 1953,
    value: 8000,
  },
 ...
 */
/* No SE_POPULATION_MULTIPLIERS: the 1991 cohort table has no SE row, so that era passes through at 1.0. */
