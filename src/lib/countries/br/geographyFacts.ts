...
/**
 * The small facts about where BR is.
 *
 * ⚠ GENERATED FROM `__snapshots__/br.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts BR --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps BR to the code and
...
export const BR_CONTINENT: Continent = "South America";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const BR_ISO_NUMERIC = "076";

export const BR_UN_MEMBER_SINCE = 1945;

export const BR_WORLD_REGION: WorldEntityRegion = "americas";

/** Where an NPP corporation is seated when it has no other home. */
export const BR_NPP_CAPITAL_STATE = "CENTRO_OESTE";

/* No BR_NON_PARTY_INDEPENDENT_BIAS: the registry has no BR entry, and
...
export const BR_MAP_ANCHOR: [number, number] = [-51.9, -14.2];
 ...
 * another country's.
 */
export const BR_MEDIAN_INCOME_THRESHOLDS: ScoreThreshold = {
  best: 35000,
  worst: 13000,
};

/** Conscription policy at seed time. */
export const BR_CONSCRIPTION: ConscriptionPolicy = {
  eligibleBand: [18, 20],
  sexEnabled: {
    male: true,
    female: false,
 ...
 * `CORE5_NORMALS.gdpGrowth.BR`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
export const BR_CORE5_NORMALS: Record<string, NormalAnchor[]> = {
  gdpGrowth: [
    {
      year: 1953,
      value: 6.5,
 ...
 *
 * ⚠ KEYED BY REGION, AND FOR BR THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const BR_ADJACENCY_MAP: AdjacencyMap = {
 ...
 */
export const BR_MAP_REGISTRY: CountryMapConfig = {
  countryId: "BR",
  name: "Brazil",
 ...
 * string.
 */
export const BR_INCOME_ANCHORS: NormalAnchor[] = [
  {
    year: 1953,
    value: 7500,
  },
 ...
 */
export const BR_POPULATION_MULTIPLIERS: Record<string, number> = {
  evangelical_conservative: 0.5,
  working_class_pt: 1.15,
  rural_agribusiness: 1.2,
  urban_middle_class: 0.8,
  urban_poor: 1.15,
  afro_brazilian: 1,
  business_financial: 0.7,
