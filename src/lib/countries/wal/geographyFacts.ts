...
/**
 * The small facts about where WAL is.
 *
 * ⚠ GENERATED FROM `__snapshots__/wal.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts WAL --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps WAL to the code and
...
export const WAL_CONTINENT: Continent = "Europe";

/**
 * ISO 3166-1 numeric -- SHARED, not owned. `ISO_NUMERIC_TO_COUNTRY` maps this
 * code back to a DIFFERENT country, because WAL has no code of its own and
 * carries its parent state's. A lookup by code will not return WAL.
 */
export const WAL_ISO_NUMERIC = "826";

export const WAL_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const WAL_NPP_CAPITAL_STATE = "";

/* No WAL_NON_PARTY_INDEPENDENT_BIAS: the registry has no WAL entry, and
...
export const WAL_MAP_ANCHOR: [number, number] = [-3.8, 52.3];
 ...
 * another country's.
 */
/* No WAL_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.WAL`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No WAL_CORE5_NORMALS: the metric-first registry carries no WAL anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR WAL THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const WAL_ADJACENCY_MAP: AdjacencyMap = {
  CDF: ["SWA", "VAL", "MWA"],
  SWA: ["CDF", "VAL", "MWA", "NWW"],
  VAL: ["CDF", "SWA", "MWA"],
 ...
 */
export const WAL_MAP_REGISTRY: CountryMapConfig = {
  countryId: "WAL",
  name: "Wales",
