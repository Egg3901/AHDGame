...
/**
 * The small facts about where SCO is.
 *
 * ⚠ GENERATED FROM `__snapshots__/sco.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts SCO --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps SCO to the code and
...
export const SCO_CONTINENT: Continent = "Europe";

/**
 * ISO 3166-1 numeric -- SHARED, not owned. `ISO_NUMERIC_TO_COUNTRY` maps this
 * code back to a DIFFERENT country, because SCO has no code of its own and
 * carries its parent state's. A lookup by code will not return SCO.
 */
export const SCO_ISO_NUMERIC = "826";

export const SCO_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const SCO_NPP_CAPITAL_STATE = "";

/* No SCO_NON_PARTY_INDEPENDENT_BIAS: the registry has no SCO entry, and
...
export const SCO_MAP_ANCHOR: [number, number] = [-4.2, 56.8];
 ...
 * another country's.
 */
/* No SCO_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.SCO`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No SCO_CORE5_NORMALS: the metric-first registry carries no SCO anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR SCO THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const SCO_ADJACENCY_MAP: AdjacencyMap = {
  GLA: ["HIG", "TAY", "CSC", "STH"],
  LOT: ["CSC", "STH"],
  HIG: ["GLA", "GRA", "TAY"],
  GRA: ["HIG", "TAY"],
 ...
 */
export const SCO_MAP_REGISTRY: CountryMapConfig = {
  countryId: "SCO",
  name: "Scotland",
 ...
 */
/* No SCO_INCOME_ANCHORS: the registry has no SCO row. */
 ...
 */
/* No SCO_POPULATION_MULTIPLIERS: the 1991 cohort table has no SCO row, so that era passes through at 1.0. */
