...
/**
 * The small facts about where DD is.
 *
 * ⚠ GENERATED FROM `__snapshots__/dd.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts DD --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps DD to the code and
...
export const DD_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const DD_ISO_NUMERIC = "278";

export const DD_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const DD_NPP_CAPITAL_STATE = "";

/* No DD_NON_PARTY_INDEPENDENT_BIAS: the registry has no DD entry, and
...
export const DD_MAP_ANCHOR: [number, number] = [13.4, 52.5];
 ...
 * another country's.
 */
/* No DD_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.DD`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No DD_CORE5_NORMALS: the metric-first registry carries no DD anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR DD THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const DD_ADJACENCY_MAP: AdjacencyMap = {
  BEO: ["BB"],
  MV: ["BB"],
  BB: ["BEO", "MV", "ST", "SN"],
  ST: ["BB", "SN", "TH"],
 ...
 */
export const DD_MAP_REGISTRY: CountryMapConfig = {
  countryId: "DD",
  name: "East Germany",
