 ...
 * are projected independently for January 2027 from the latest registration
 * observations. Changing another era's seed must never alter 2027.
 */

/**
 * US party registration + organization seed for the 2027-default preset.
 *
 * Anchored to 2022 state voter-registration data + 2022 midterm election
 * results. The 2027 projection retains these documented modern-era shifts:
 *
...
import type { StateRegistrationSeed } from "@/lib/seeds/registration/registrationLanes";
...
const US_ASSIGNMENTS_2027: Record<keyof typeof US_LANES_2027, string[]> = {
...
// Per-state overrides for jurisdictions with notable modern registration
// characteristics that diverge from their lane template.
const US_OVERRIDES_2027: Record<string, Omit<StateRegistrationSeed, "countryId" | "stateId">> = {
...
function buildUSSeeds2023(): StateRegistrationSeed[] {
  ...
  for (const [lane, states] of Object.entries(US_ASSIGNMENTS_2027) as Array<
    [keyof typeof US_LANES_2027, string[]]
  >) {
    const template = US_LANES_2027[lane];
    for (const stateId of states) {
      const override = US_OVERRIDES_2027[stateId];
      seeds.push({
        countryId: "US",
        stateId,
  ...
  for (const [stateId, override] of Object.entries(US_OVERRIDES_2027)) {
...
/**
 * Returns the US 2027-default registration / org seed bundle.
 * US-only: non-US countries in the 2023 preset fall back to their 2019 data.
 */
export function build2027RegistrationSeeds(): StateRegistrationSeed[] {
