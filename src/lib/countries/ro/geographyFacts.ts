...
/**
 * The small facts about where RO is.
 *
 * ⚠ GENERATED FROM `__snapshots__/ro.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts RO --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps RO to the code and
...
export const RO_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const RO_ISO_NUMERIC = "642";

export const RO_UN_MEMBER_SINCE = 1955;

export const RO_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const RO_NPP_CAPITAL_STATE = "";

/* No RO_NON_PARTY_INDEPENDENT_BIAS: the registry has no RO entry, and
...
export const RO_MAP_ANCHOR: [number, number] = [24.9, 45.9];
 ...
 * another country's.
 */
/* No RO_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.RO`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No RO_CORE5_NORMALS: the metric-first registry carries no RO anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR RO THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const RO_ADJACENCY_MAP: AdjacencyMap = {
  RO_BUC: ["RO_MUN"],
 ...
 */
export const RO_MAP_REGISTRY: CountryMapConfig = {
  countryId: "RO",
  name: "Romania",
 ...
 */
/* No RO_INCOME_ANCHORS: the registry has no RO row. */
 ...
 */
/* No RO_POPULATION_MULTIPLIERS: the 1991 cohort table has no RO row, so that era passes through at 1.0. */
