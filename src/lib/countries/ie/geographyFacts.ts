...
/**
 * The small facts about where IE is.
 *
 * ⚠ GENERATED FROM `__snapshots__/ie.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts IE --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps IE to the code and
...
export const IE_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const IE_ISO_NUMERIC = "372";

export const IE_UN_MEMBER_SINCE = 1955;

export const IE_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const IE_NPP_CAPITAL_STATE = "DUB";

/* No IE_NON_PARTY_INDEPENDENT_BIAS: the registry has no IE entry, and
...
export const IE_MAP_ANCHOR: [number, number] = [-8.2, 53.4];
 ...
 * another country's.
 */
export const IE_MEDIAN_INCOME_THRESHOLDS: ScoreThreshold = {
  best: 52000,
  worst: 19000,
};

/** Conscription policy at seed time. */
export const IE_CONSCRIPTION: ConscriptionPolicy = {
  eligibleBand: [18, 20],
  sexEnabled: {
    male: true,
    female: false,
 ...
 * `CORE5_NORMALS.gdpGrowth.IE`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
export const IE_CORE5_NORMALS: Record<string, NormalAnchor[]> = {
  gdpGrowth: [
    {
      year: 1953,
      value: 2,
 ...
 *
 * ⚠ KEYED BY REGION, AND FOR IE THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const IE_ADJACENCY_MAP: AdjacencyMap = {
  DUB: ["KIL"],
  KIL: ["DUB", "MID", "WEX", "DON"],
 ...
 */
export const IE_MAP_REGISTRY: CountryMapConfig = {
  countryId: "IE",
  name: "Ireland",
