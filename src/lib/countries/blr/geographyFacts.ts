...
/**
 * The small facts about where BLR is.
 *
 * ⚠ GENERATED FROM `__snapshots__/blr.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts BLR --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps BLR to the code and
...
export const BLR_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const BLR_ISO_NUMERIC = "112";

export const BLR_UN_MEMBER_SINCE = 1945;

export const BLR_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const BLR_NPP_CAPITAL_STATE = "BLR_MIN";

/* No BLR_NON_PARTY_INDEPENDENT_BIAS: the registry has no BLR entry, and
...
export const BLR_MAP_ANCHOR: [number, number] = [27.9, 53.5];
 ...
 * another country's.
 */
/* No BLR_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.BLR`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No BLR_CORE5_NORMALS: the metric-first registry carries no BLR anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR BLR THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const BLR_ADJACENCY_MAP: AdjacencyMap = {
 ...
 */
export const BLR_MAP_REGISTRY: CountryMapConfig = {
  countryId: "BLR",
  name: "Belarus",
 ...
 */
/* No BLR_INCOME_ANCHORS: the registry has no BLR row. */
 ...
 */
/* No BLR_POPULATION_MULTIPLIERS: the 1991 cohort table has no BLR row, so that era passes through at 1.0. */
