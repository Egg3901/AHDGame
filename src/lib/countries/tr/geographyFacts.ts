...
/**
 * The small facts about where TR is.
 *
 * ⚠ GENERATED FROM `__snapshots__/tr.pre-move.json`, NOT TRANSCRIBED.
 * Regenerate with:
 *
 *     npx tsx scripts/countries/gen-country-geography-facts.ts TR --force
 *
 ...
 * `COUNTRY_TO_ISO_NUMERIC` maps TR to the code and
...
export const TR_CONTINENT: Continent = "Europe";

/** ISO 3166-1 numeric. `ISO_NUMERIC_TO_COUNTRY` holds the inverse entry. */
export const TR_ISO_NUMERIC = "792";

export const TR_UN_MEMBER_SINCE = 1945;

export const TR_WORLD_REGION: WorldEntityRegion = "europe";

/** Where an NPP corporation is seated when it has no other home. */
export const TR_NPP_CAPITAL_STATE = "TR_ANK";

/* No TR_NON_PARTY_INDEPENDENT_BIAS: the registry has no TR entry, and
...
export const TR_MAP_ANCHOR: [number, number] = [35.2, 39];
 ...
 * another country's.
 */
/* No TR_MEDIAN_INCOME_THRESHOLDS: no row in the registry. */

/**
 * Core-5 metric normals, keyed by metric.
 *
 * ⚠ THE REGISTRY IS METRIC-FIRST, NOT COUNTRY-FIRST. `CORE5_NORMALS` is read as
 * `CORE5_NORMALS.gdpGrowth.TR`, so the snapshot captured one entry per
 * metric rather than one object for the country. Each anchor is keyed `year`,
 * a NUMBER -- not `era`, a string.
 */
/* No TR_CORE5_NORMALS: the metric-first registry carries no TR anchors. */

/**
 * Which regions border which.
 *
 * ⚠ KEYED BY REGION, AND FOR TR THOSE KEYS ARE NOT COUNTRY CODES. In the
 ...
 */
export const TR_ADJACENCY_MAP: AdjacencyMap = {
 ...
 */
export const TR_MAP_REGISTRY: CountryMapConfig = {
  countryId: "TR",
  name: "Turkey",
 ...
 * string.
 */
export const TR_INCOME_ANCHORS: NormalAnchor[] = [
  {
    year: 1953,
    value: 1900,
  },
 ...
 */
/* No TR_POPULATION_MULTIPLIERS: the 1991 cohort table has no TR row, so that era passes through at 1.0. */
