...
import { getPartyIdString, getStatePartyOrgDocumentId } from "@/lib/db/partyLookup";
...
export async function ensureStatePartyOrgRow(
  ...
  if (existing) {
    // Heal a legacy/malformed row whose scope fields drifted from its canonical
    // `_id` — most importantly a missing `countryId` (older leadership upserts
    // created rows without it), which makes the row invisible to every
    // countryId-scoped query, so the PS spend fails with "missing-row". The
    // `_id` is the source of truth for (stateId, partyId), so repair to match.
    const patch: Partial<StatePartyOrg> = {};
    if (existing.countryId !== args.countryId) patch.countryId = args.countryId;
    if (existing.stateId !== args.stateId) patch.stateId = args.stateId;
    if (existing.partyId !== partyId) patch.partyId = partyId;
