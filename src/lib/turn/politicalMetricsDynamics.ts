...
export async function processPoliticalMetricsDynamics(
  ...
  const [spendingRollup, approvalByCountry, sectorTax, eraGameState] = await Promise.all([
    spendingProvider(db),
  ...
  for (const countryId of countryIds) {
    ...
    const cabinetContribution = await getPoliticalCabinetContribution(db, countryId);
    ...
    for (const m of macroDocs) {
      ...
      for (const category of ["economic", "population"] as const) {
        const cat = (m as unknown as Record<string, Record<string, { value?: number }>>)[category];
        if (!cat) continue;
    ...
    for (const record of regionalRecords) {
      ...
      levels.set(record.legislationTypeId, Math.max(0, Math.min(4, record.policyOptionIndex ?? 0)));
    ...
    for (const doc of docs) {
      ...
      if (!residuals) {
        ...
        for (const [metricId, points] of Object.entries(national)) {
          ...
          // self-heal recomputes from.
          const target = composeTarget(
            points,
            supplement?.[id] ?? 0,
            structural + macroTerm + engineTerm + cabinetOf(id)
          );
      ...
      if (healed || changed || cabinetChanged) {
        ops.push({
          updateOne: {
            filter: { _id: doc._id },
            update: {
