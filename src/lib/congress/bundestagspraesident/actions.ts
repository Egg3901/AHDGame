/**
 * Bundestagspräsident election action handlers — DE parallel of
 * `src/lib/congress/speaker/actions.ts`. Same shape (declare / withdraw /
 * vote / start_election / force_end / reset_election), same 24-hour voting
 * window. Eligibility follows the role's RoleEligibilityPolicy — currently
 * `any-seated`, meaning any seated MdB may declare and vote. NPPs do not
 * nominate or vote.
 */
...
import { resolveBundestagspraesidentElection } from "./resolveElection";
import type {
...
export interface ActionContext {
  ...
  bundestag: Awaited<ReturnType<typeof getBundestagComposition>>;
  authUser: { userId: string; isAdmin?: boolean };
  action: string;
  nominationId?: string;
}

export async function handleBundestagspraesidentAction(ctx: ActionContext): Promise<ActionResult> {
  const { action } = ctx;

  if (action === "start_election") return handleStartElection(ctx);
  if (action === "force_end") return handleForceEnd(ctx);

  // declare, withdraw, vote require character + Bundestag membership
  const { db, bundestag, authUser, nominationId } = ctx;
  ...
  if (!myOfficial) {
    return {
      success: false,
      error: "Only sitting Bundestag members can participate in Bundestagspräsident elections.",
      status: 403,
    };
  }
  ...
  if (action === "declare") {
    ...
    if (!isEligibleParty) {
      return {
        success: false,
        error: `Only ${eligibilityLabel} may declare for Bundestagspräsident.`,
        status: 403,
      };
    }
    ...
    return { success: true, message: "You have declared for Bundestagspräsident." };
  }
  ...
  if (action === "withdraw") {
    ...
    return { success: true, message: "Withdrawn from Bundestagspräsident election." };
  }
  ...
  if (action === "vote") {
    ...
    if (!isEligibleParty) {
      return {
        success: false,
        error: `Only ${eligibilityLabel} may vote for Bundestagspräsident.`,
        status: 403,
      };
    }
    if (!nominationId) {
...
async function handleStartElection(ctx: ActionContext): Promise<ActionResult> {
  ...
  return {
    success: true,
    message: `Bundestagspräsident election opened (closes ${endsAt.toISOString()}).`,
  };
}
...
async function handleForceEnd(ctx: ActionContext): Promise<ActionResult> {
  ...
  const resolved = await resolveBundestagspraesidentElection(db, partyMap, true);
  if (!resolved) {
