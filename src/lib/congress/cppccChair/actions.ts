/**
 * Chairman of the CPPCC election action handlers — CN parallel of the NPCSC
 * Chair handlers (src/lib/congress/npcscChair/actions.ts). Same shape (declare
 * / withdraw / vote / start_election / force_end), same 24-hour window.
 * Eligibility is `largest-single-party` — only seated NPC delegates of the
 * largest NPC party (the CCP) may declare and vote. NPPs do not participate.
 */
...
import { resolveCppccChairElection } from "./resolveElection";
import type {
  CppccChairElection,
...
export async function handleCppccChairAction(ctx: ActionContext): Promise<ActionResult> {
  const { action } = ctx;
  ...
  if (!myOfficial) {
    return {
      success: false,
      error: "Only sitting NPC delegates can participate in CPPCC Chairman elections.",
      status: 403,
    };
  }
  ...
  if (action === "declare") {
    ...
    if (!isEligibleParty) {
      return {
        success: false,
        error: `Only ${eligibilityLabel} may declare for Chairman of the CPPCC.`,
        status: 403,
      };
    }
    ...
    return { success: true, message: "You have declared for Chairman of the CPPCC." };
  }
  ...
  if (action === "withdraw") {
    ...
    return { success: true, message: "Withdrawn from the CPPCC Chairman election." };
  }
  ...
  if (action === "vote") {
    ...
    if (!isEligibleParty) {
      return {
        success: false,
        error: `Only ${eligibilityLabel} may vote for Chairman of the CPPCC.`,
        status: 403,
      };
    }
    if (!nominationId) {
    ...
    await db.collection<CppccChairNomination>("cppccChairNominations").updateMany(
    ...
    await db.collection<CppccChairNomination>("cppccChairNominations").updateOne(
      { _id: oid },
      {
        $inc: { votesFor: 1 },
...
async function handleStartElection(ctx: ActionContext): Promise<ActionResult> {
  ...
  return {
    success: true,
    message: `Chairman of the CPPCC election opened (closes ${endsAt.toISOString()}).`,
  };
}
...
async function handleForceEnd(ctx: ActionContext): Promise<ActionResult> {
  ...
  const resolved = await resolveCppccChairElection(db, partyMap, true);
  if (!resolved) {
