...
describe("JP 2019 calibration — Japan 2017 HR (Abe-era LDP dominance; urban CDP lean) anchor", () => {
  const leans = deriveRegionLeans("JP", "2019");
  const byId = Object.fromEntries(leans.map((l) => [l.regionId, l]));

  it("passes the calibration target (loss 0)", () => {
    const r = evaluateCell("JP", "2019")!;
