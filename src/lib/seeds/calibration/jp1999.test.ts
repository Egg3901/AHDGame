...
describe("JP 1999 calibration — Japan 2000 HR (lost decade; DPJ founding, 1998 UH rout) anchor", () => {
  const leans = deriveRegionLeans("JP", "1999");
  const byId = Object.fromEntries(leans.map((l) => [l.regionId, l]));

  it("passes the calibration target (loss 0)", () => {
    const r = evaluateCell("JP", "1999")!;
