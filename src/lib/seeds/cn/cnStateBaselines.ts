...
import { cnStateMetrics } from "./cnStateMetrics";

/**
 * China macro-region metric baselines — the "resting" value each metric
 * decays toward when no policy intervention is active.
 *
 * Derived from cnStateMetrics so a freshly seeded game has zero decay
 * pressure (baselines match current values 1:1). Any metric that changes
 * in cnStateMetrics automatically updates the baseline here.
 */
...
export const cnStateBaselines: StateMetricBaseline[] = cnStateMetrics.map(toBaseline);
