...
import { deStateMetrics } from "./deStateMetrics";

/**
 * German Bundesländer metric baselines — the "resting" value each metric
 * decays toward when no policy intervention is active.
 *
 * Derived from deStateMetrics so a freshly seeded game has zero decay
 * pressure (baselines match current values 1:1). Any metric that changes
 * in deStateMetrics automatically updates the baseline here.
 */
...
export const deStateBaselines: StateMetricBaseline[] = deStateMetrics.map(toBaseline);
