...
describe("buildBlendRegionCards", () => {
  ...
  it("counts an omitted candidate seen in both the tally and polling exactly once", () => {
    const race = senateRace({
      candidates: [
    ...
    expect(card.omittedLabel).toBe("+1 more candidate");
  });

  it("pluralizes the omitted label and reports zero when the field is whole", () => {
    const race = senateRace({
      generalTally: { totalVotes: { s1: 30_000, s2: 20_000, x1: 5, x2: 6 }, turnSnapshots: [] },
    });
    const [card] = buildBlendRegionCards({ ...baseInput, elections: [race] });
    expect(card.omittedCount).toBe(2);
    expect(card.omittedLabel).toBe("+2 more candidates");
    const whole = buildBlendRegionCards({ ...baseInput, elections: [senateRace()] })[0];
    expect(whole.omittedCount).toBe(0);
    expect(whole.omittedLabel).toBeNull();
  });

  it("keeps one race's extra ids out of another race's omitted count", () => {
    const cards = buildBlendRegionCards({
      ...baseInput,
      elections: [
        senateRace(),
        houseRace({
          generalTally: { totalVotes: { c1: 60_000, c2: 40_000, s9: 7 }, turnSnapshots: [] },
        }),
      ],
    });
    expect(cards[0].omittedCount).toBe(0);
    expect(cards[1].omittedCount).toBe(1);
  });

  it("treats missing polling and tally maps as no omissions, not a crash", () => {
    const [card] = buildBlendRegionCards({
      ...baseInput,
      elections: [senateRace({ polling: undefined, generalTally: undefined })],
    });
    expect(card.omittedCount).toBe(0);
    expect(card.omittedLabel).toBeNull();
  });

  it("counts a region's own extra ids, not nationwide leftovers, on a region card", () => {
    const pres = houseRace({
      id: "e-pres",
      electionType: "president",
      state: "US",
      generalTally: {
        totalVotes: { c1: 60_000, c2: 40_000, nationalOnly: 9_000 },
        turnSnapshots: [],
      },
    });
    const [card] = buildBlendRegionCards({
      ...
      elections: [pres],
      regionCode: "GA",
      regionVotesByElectionId: { "e-pres": { c1: 30_000, c2: 70_000, regionalExtra: 4_000 } },
      regionElectoralVotes: 16,
    });
    expect(card.omittedCount).toBe(1);
    expect(card.omittedLabel).toBe("+1 more candidate");
