...
describe("buildBlendDetail — the seat arithmetic", () => {
  ...
  it("keeps votes, whole quotas and remainder consistent when the gate limits seats", () => {
    // 62,000 votes at a 10,000 quota is 6.2 quotas, but the eligibility gate
    // can leave this candidate with a single seat.
...
describe("buildBlendDetail — proportional Commons copy", () => {
  it("keeps the Hare quota and remainder arithmetic visible", () => {
    const m = buildBlendDetail(houseInput({ seatsEstimate: { c1: 1, c2: 9 } }));
    expect(m.facts.some((f) => f.key === "Quota")).toBe(true);
    expect(m.rows.every((r) => r.math.some((x) => x.key === "Quota"))).toBe(true);
    expect(m.rows.every((r) => r.math.some((x) => x.key === "Whole quotas"))).toBe(true);
    expect(m.standfirst).toContain("Hare quota");
    expect(m.hemiNote).toContain("whole quotas");
  });

  it("does not mention a historical winner bonus", () => {
    const m = buildBlendDetail(houseInput({ seatsEstimate: { c1: 1, c2: 9 } }));
    expect(m.standfirst).not.toContain("winner's bonus");
    expect(m.hemiNote).not.toContain("winner's bonus");
    expect(m.rows.every((r) => !r.mathNote.includes("winner's bonus"))).toBe(true);
  });

  it("keeps em and en dashes out of the generated copy", () => {
    const m = buildBlendDetail(houseInput({ seatsEstimate: { c1: 1, c2: 9 } }));
