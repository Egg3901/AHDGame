...
/**
 * Seeds Austria's national budget, default enacted laws (from the at_*
 * legislation defaults), per-region state budgets, and the sovereign issuer.
 */
export async function seedAtBudgets(
  db: Db,
  reset: boolean,
  log: (msg: string) => void,
  preset: string
  ...
  if (reset) {
    await db.collection<FederalBudget>("federalBudget").deleteMany({ countryId: "AT" });
    const atStateIds = (
      await db.collection("states").find({ countryId: "AT" }).project({ _id: 1 }).toArray()
    ).map((s) => s._id as string);
    if (atStateIds.length > 0) {
      await db
        .collection<StateBudget>("stateBudgets")
        .deleteMany({ stateId: { $in: atStateIds }, countryId: "AT" });
    }
    await db.collection<EnactedLaw>("enactedLaws").deleteMany({ countryId: "AT" });
    log("Reset: deleted AT budget data");
  }
  ...
  const atFiscalYear =
    getNationalBudgetSeedConfigsForPreset(preset).find((c) => c.countryId === "AT")?.fiscalYear ??
    1979;
  const atNationalBudget = presetBudgets.find((b) => b.countryId === "AT");
  if (!atNationalBudget) {
    log(`[AT] no national budget config for preset ${preset} — skipping`);
    return;
  }
  {
    const { _id, ...budgetData } = atNationalBudget;
    await db
      .collection<FederalBudget>("federalBudget")
      .updateOne({ _id }, { $set: { ...budgetData, updatedAt: new Date() } }, { upsert: true });
    log("Seeded 1 AT national budget");
  }

  const atEnactedLaws = generateDefaultEnactedLaws(preset).filter((law) => law.countryId === "AT");
  for (const law of atEnactedLaws) {
  ...
  log(`Seeded ${atEnactedLaws.length} AT default enacted laws`);

  const states = await db.collection<State>("states").find({ countryId: "AT" }).toArray();
  ...
  const stateBudgets = generateStateBudgets(statesForBudgets, atFiscalYear);
  ...
  log(`Seeded ${stateBudgets.length} AT regional budgets`);

  const countryOwnedSeedData = generateCountryOwnedSeedData(statesForBudgets, preset);
  const atCorpData = countryOwnedSeedData.filter(
    (entry) => entry.corporation.countryOwnerId === "AT"
  );
  for (const entry of atCorpData) {
  ...
  if (atCorpData.length > 0) log(`Seeded ${atCorpData.length} AT sovereign issuer setup(s)`);
}
