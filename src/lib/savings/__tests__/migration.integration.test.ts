...
function world(mode: "shadow" | "authoritative", readCurrencies: string[] = []): InMemoryDb {
  ...
  db.seed("centralBanks", [
    { _id: "US", countryId: "US", externalBroadMoney: 10_000, bankReserveRequirement: 0.1 },
  ]);
  db.seed("corporations", [
    ...
    },
    {
      _id: DEAD,
      name: "Dead Bank",
      bankCharter: { type: "retail", status: "revoked", currency: "USD", cashReserves: 0 },
    },
  ]);
  db.seed("characters", [
    {
      ...
      currencyBalances: {
        ...
        pendingSavingsInterest: { USD: 1.5 },
      },
    },
    { _id: P2, name: "Two", currencyBalances: { savings: { USD: 300 } } },
    {
      _id: P3,
      name: "Three",
      currencyBalances: { savings: { USD: 50 }, savingsHolder: { USD: DEAD.toString() } },
    },
  ]);
  return db;
}
...
describe("runSavingsMigration", () => {
  ...
  it("creates accounts, moves backing, recognizes liabilities, reconciles, and resumes idempotently", async () => {
    ...
    expect(
      (
        db.collection("characters").docs[2] as {
          currencyBalances: { savingsHolder: { USD: string } };
        }
      ).currencyBalances.savingsHolder.USD
    ).toBe("centralBank");
    ...
    const savings = (
      db.collection("characters").docs as { currencyBalances: { savings: { USD: number } } }[]
    ).map((c) => c.currencyBalances.savings.USD);
    expect(savings).toEqual([700, 300, 50]);

    const second = await runSavingsMigration(db as unknown as Db, 91);
    expect(second.batches[0]).toMatchObject({
      applied: 0,
      replayed: 3,
      failed: 0,
      reconciled: true,
