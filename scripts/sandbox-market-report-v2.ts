/**
 * Sandbox Market Status Report v2
 * Queries the sandbox DB for corporation and commodity market imbalances.
 *
 * Usage: npx tsx scripts/sandbox-market-report-v2.ts
 */
...
async function main() {
  ...
  const nppCorps = corps.filter((c) => c.ceoType === "npp");
  const playerCorps = corps.filter((c) => c.ceoType === "character" || !c.ceoType);
  const imperialCorps = corps.filter((c) => c.ceoType === "imperial");
  const natCorps = corps.filter((c) => c.countryOwnerId);

  console.log(`- Player corps: ${playerCorps.length}`);
  console.log(`- NPP corps: ${nppCorps.length}`);
  console.log(`- Imperial corps: ${imperialCorps.length}`);
  console.log(`- Nationalized / state-owned: ${natCorps.length}\n`);
  ...
  console.log(
    "| # | Name | Ticker | Share Price | Total Shares | Market Cap | Liquid Capital | Country | Type |"
  );
  console.log(
    "|---|------|--------|-------------|--------------|------------|----------------|---------|------|"
  );
  sortedByPrice.slice(0, 15).forEach((c, i) => {
    ...
    const type = c.ceoType || "character";
    console.log(
      `| ${i + 1} | ${c.name} | ${c.tickerSymbol || "—"} | $${price.toFixed(2)} | ${shares.toLocaleString()} | $${mcap.toLocaleString()} | $${liq.toLocaleString()} | ${c.countryId || "?"} | ${type} |`
    );
  });
  ...
  if (zeroPrice.length > 0) {
    ...
    zeroPrice.forEach((c) =>
      console.log(`  - ${c.name} (${c.tickerSymbol || "no ticker"}) — ${c.ceoType || "character"}`)
    );
  }
  ...
  console.log("| Name | Ticker | Market Cap | Liquid Capital | Ratio | Type |");
  console.log("|------|--------|------------|----------------|-------|------|");
  ratios.slice(0, 10).forEach((r) => {
    console.log(
      `| ${r.name} | ${r.ticker || "—"} | $${r.mcap.toLocaleString()} | $${r.liq.toLocaleString()} | ${r.ratio.toFixed(1)}x | ${r.ceoType || "character"} |`
    );
  });
  ...
  // Negative liquid capital
  const negativeLiq = corps.filter((c) => (c.liquidCapital || 0) < 0);
  if (negativeLiq.length > 0) {
    console.log(`\n### Negative Liquid Capital: ${negativeLiq.length}\n`);
    negativeLiq.forEach((c) => {
      const mcap = (c.sharePrice || 0) * (c.totalShares || 0);
      console.log(
        `  - ${c.name} (${c.tickerSymbol || "no ticker"}): liquid=$${c.liquidCapital.toLocaleString()}, mcap=$${mcap.toLocaleString()}, type=${c.ceoType || "character"}`
      );
    });
  }
  ...
  if (vacantCeos.length > 0) {
    ...
    vacantCeos.forEach((c) =>
      console.log(
        `  - ${c.name} (${c.tickerSymbol || "no ticker"}) — vacant since turn ${c.ceoVacantSinceTurn || "?"}, type=${c.ceoType || "character"}`
      )
    );
  }
  ...
  if (sectors.length > 0) {
    ...
    sectors.forEach((s) => {
      ...
      byCountry[s.countryId] = (byCountry[s.countryId] || 0) + 1;
    });
    ...
    // Corp sector ownership
    const corpSectorCounts: Record<string, { name: string; count: number; ceoType: string }> = {};
    for (const s of sectors) {
      const cid = s.corporationId.toString();
      if (!corpSectorCounts[cid]) {
        const corp = corps.find((c) => c._id.toString() === cid);
        corpSectorCounts[cid] = {
          name: corp?.name || "?",
          count: 0,
          ceoType: corp?.ceoType || "character",
        };
      }
      corpSectorCounts[cid].count++;
    }
    const sortedBySectors = Object.entries(corpSectorCounts)
      .sort(([, a], [, b]) => b.count - a.count)
      .map(([, v]) => v);

    console.log("\n### Top 15 Corps by Sector Count\n");
    console.log("| # | Corp Name | Sectors | Type |");
    console.log("|---|-----------|---------|------|");
    sortedBySectors.slice(0, 15).forEach((c, i) => {
      console.log(`| ${i + 1} | ${c.name} | ${c.count} | ${c.ceoType} |`);
    });

    // NPP sector ownership
    const nppSectorCounts = sortedBySectors.filter((c) => c.ceoType === "npp");
    console.log(`\n### NPP Corps with Sectors: ${nppSectorCounts.length}\n`);
    console.log("| Corp Name | Sectors |");
    console.log("|-----------|---------|");
    nppSectorCounts.slice(0, 20).forEach((c) => {
      console.log(`| ${c.name} | ${c.count} |`);
    });
    if (nppSectorCounts.length > 20) {
      console.log(`... and ${nppSectorCounts.length - 20} more`);
    }
