// Upload Wikimedia-sourced bento card images to R2.
//   railway run --service "Main Site" node scripts/upload-landing-images-to-r2.mjs
...
const dir = path.join(root, "scripts", "landing-images");
const files = readdirSync(dir).filter((f) => f.endsWith(".webp"));
if (files.length === 0) {
  console.error(`No .webp files found in ${dir}. Run the image-sourcing step first.`);
  process.exit(1);
}
for (const file of files) {
  const slug = path.basename(file, ".webp");
  const key = `static/landing/${slug}.webp`;
