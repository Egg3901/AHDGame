  ...
  const [resources, elections, electionCandidates, budgets, stateBudgets, policies, laws, news] =
    await Promise.all([
      db
        ...
        .sort({ _id: 1 })
        .toArray(),
      db.collection("elections").find({ countryId }).sort({ _id: 1 }).toArray(),
      db.collection("electionCandidates").find({ countryId }).sort({ _id: 1 }).toArray(),
      db.collection("federalBudget").find({ countryId }).sort({ _id: 1 }).toArray(),
      db.collection("stateBudgets").find({ countryId }).sort({ _id: 1 }).toArray(),
      db.collection("countryGameStates").find({ countryId }).sort({ _id: 1 }).toArray(),
      db.collection("enactedLaws").find({ countryId }).sort({ _id: 1 }).toArray(),
      db.collection("newsPosts").find({ countryId }).sort({ _id: 1 }).toArray(),
    ]);
