 ...
 *
 * Requires a mongod on 127.0.0.1:27018 and is skipped by default; set
 * AHD_SP_SNAPSHOT_REAL_MONGO=1 to run it.
 */
...
// Real-Mongo suites skip unless explicitly enabled, matching the convention in
// the other *.mongo.test.ts files here. CI provides no mongod, so without this
// gate the suite fails with ECONNREFUSED instead of reporting that it needs a
// database to be useful.
const RUN_REAL_MONGO = process.env.AHD_SP_SNAPSHOT_REAL_MONGO === "1";

it.skipIf(!RUN_REAL_MONGO)(
  "exports a versioned local SP world after a real 1953 US turn without account secrets",
  async () => {
